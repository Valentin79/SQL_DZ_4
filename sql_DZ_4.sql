...
FROM shops s
INNER JOIN cats c ON s.id = c.shops_id
WHERE c.name = "Murzik";

-- 2.2
SELECT s.shopname, c.name
FROM shops s
INNER JOIN (SELECT name, shops_id
			FROM cats
            WHERE `name` = "Murzik")
c ON s.id = c.shops_id;

-- 3 Вывести магазины, в котором НЕ продаются коты “Мурзик” и “Zuza”
SELECT s.shopname, c.name
FROM shops s
INNER JOIN cats c ON s.id = c.shops_id
WHERE NOT c.name = "Murzik" AND NOT c.name = "Zuza";
...
-- 4.1 Вывести название и цену для всех анализов, которые продавались 5 февраля 
